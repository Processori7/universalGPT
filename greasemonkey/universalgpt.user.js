// ==UserScript==
// @name                   UniversalGPT 🤖
// @description            Add AI answers to Yandex, DuckDuckGo, Brave, Google, Bing (powered by GPT-4o!)
...
// @description:ru         Добавляет ответы ИИ в Yandex (на базе GPT-4o!)
...
// @match                  *://nova.rambler.ru/search*
// @match                  *://rambler.ru/search*
// @match                  *://*.rambler.ru/search*
// @match                  *://go.mail.ru/search*
// @match                  *://mail.ru/search*
...
// @noframes
// @downloadURL            https://github.com/Processori7/universalGPT/raw/main/greasemonkey/yandexgpt.user.js
// @updateURL              https://github.com/Processori7/universalGPT/raw/main/greasemonkey/yandexgpt.user.js
// @homepageURL            https://github.com/Processori7/universalGPT
// @supportURL             https://github.com/Processori7/universalGPT/issues
// @contributionURL        https://github.com/Processori7/universalGPT
// ==/UserScript==
...
(async () => {
        ...
        msgs: await new Promise(resolve => {
            ...
            function handleMsgs(resp) {
                try { // to return localized messages.json
                    ...
                    // Ensure visible branding uses UniversalGPT instead of DuckDuckGPT
                    Object.keys(flatMsgs).forEach(k => {
                        if (typeof flatMsgs[k] == 'string')
                            flatMsgs[k] = flatMsgs[k].replace(/DuckDuckGPT/g, 'UniversalGPT')
                    })
    ...
    app.name = 'UniversalGPT'
    app.slug = 'universalgpt'
    app.configKeyPrefix = 'universalgpt'
    
    // Detect search engine
    const hostname = location.hostname
    app.site = hostname.includes('yandex') || hostname.includes('ya.ru') ? 'yandex'
             : hostname.includes('duckduckgo') ? 'duckduckgo'
             : hostname.includes('brave') ? 'brave'
             : hostname.includes('google') ? 'google'
             : hostname.includes('bing') ? 'bing'
             : (hostname.includes('rambler.ru') || hostname.includes('nova.rambler')) ? 'rambler'
             : hostname.includes('mail.ru') ? 'mailru'
             : hostname.includes('yahoo') ? 'yahoo'
             : hostname.includes('ecosia') ? 'ecosia'
             : hostname.includes('startpage') ? 'startpage'
             : 'unknown'
    
    // Site-specific config
    const siteConfig = {
        yandex: { selector: 'body', queryParam: 'text', isCentered: false },
        duckduckgo: { selector: env.browser.isMobile ? '[data-area*=mainline]' : '[class*=sidebar]', queryParam: 'q', isCentered: !!document.documentElement.className.includes('center') },
        brave: { selector: 'body', queryParam: 'q', isCentered: false },
        google: { selector: 'body', queryParam: 'q', isCentered: false },
        bing: { selector: 'body', queryParam: 'q', isCentered: false },
        rambler: { selector: 'body', queryParam: 'query', isCentered: false },
        mailru: { selector: 'body', queryParam: 'text', isCentered: false },
        yahoo: { selector: 'body', queryParam: 'p', isCentered: false },
        ecosia: { selector: 'body', queryParam: 'q', isCentered: false },
        startpage: { selector: 'body', queryParam: 'query', isCentered: false }
    }
    ...
    // Override important URLs; keep contributors pointing to original project's contributors
    Object.assign(app.urls, {
        github: 'https://github.com/Processori7/universalGPT',
        contributors: 'https://github.com/KudoAI/duckduckgpt/graphs/contributors',
        app: 'https://github.com/Processori7/universalGPT',
        publisher: 'https://github.com/Processori7',
        support: 'https://github.com/Processori7/universalGPT/issues',
        discuss: 'https://github.com/Processori7/universalGPT/discussions',
        update: Object.assign(app.urls.update || {}, { gm: 'https://github.com/Processori7/universalGPT/raw/main/greasemonkey/universalgpt.user.js' })
    })
    app.alerts = {
    ...
    Object.assign(settings, { controls: { // displays top-to-bottom, left-to-right in Settings modal
        proxyAPIenabled: { type: 'toggle', icon: 'sunglasses', defaultVal: true,
        ...
        autoScroll: { type: 'toggle', mobile: false, icon: 'arrowsDown', defaultVal: true,
    ...
    window.show = {
        ...
        reply({ content, standby = false, apiUsed = null }) {
            ...
            if (!app.div.querySelector('.reply-pre')) {
                ...
                // Create/append title
                const appHeaderLogo = logos.universalgpt.create()
                ...
                if (aboutBtn) {
                    aboutBtn.onclick = (event) => { 
                        ...
                        try { modals.open('about') } catch(e) { console.error('About error:', e) }
                    }
                }
                if (settingsBtn) {
                    settingsBtn.onclick = (event) => { 
                        ...
                        event.stopPropagation();
                        try {
                            const settingsModal = modals.settings.show()
                            if (settingsModal) {
                                modals.stack.unshift('settings')
                                modals.init(settingsModal)
                            }
                        } catch(e) { console.error('Settings error:', e) }
                    }
                }
                if (fontSizeBtn) {
                    fontSizeBtn.onclick = (event) => { 
                        event.preventDefault();
                        event.stopPropagation();
                        try { fontSizeSlider.toggle() } catch(e) { console.error('Font size error:', e) }
                    }
                }
                if (chevronBtn) {
                    chevronBtn.onclick = (event) => { 
                        event.preventDefault();
                        event.stopPropagation();
                        try { toggle.minimized() } catch(e) { console.error('Chevron error:', e) }
                    }
                }
                if (pinBtn) {
                    pinBtn.onclick = (event) => { 
                        event.preventDefault();
                        event.stopPropagation();
                        try { toggle.anchorMode() } catch(e) { console.error('Pin error:', e) }
                    }
                }
                if (wsbBtn) {
                    wsbBtn.onclick = (event) => { 
                        event.preventDefault();
                        event.stopPropagation();
                        try { toggle.sidebar('wider') } catch(e) { console.error('WSB error:', e) }
                    }
                }
                if (arrowsBtn) {
                    arrowsBtn.onclick = (event) => { 
                        event.preventDefault();
                        event.stopPropagation();
                        try { toggle.expandMode() } catch(e) { console.error('Arrows error:', e) }
                    }
                }

                // Create/append 'by Processor'
                const bylineSpan = dom.create.elem('span', { class: 'byline no-user-select', textContent: 'by ' })
                bylineSpan.append(dom.create.anchor('https://github.com/Processori7', 'Processor'))
    ...
    const logos = { // requires dom.js + <app|env> + GM_getResourceText()
        universalgpt: {
            ...
            create() { // requires dom.js + app
                const universalgptLogo = dom.create.elem('img', { id: `${app.slug}-logo`, class: 'no-mobile-tap-outline' })
                logos.universalgpt.update(universalgptLogo)
                return universalgptLogo
            },
    ...
    // Create alias for icons.universalgpt to point to logos.universalgpt
    if (typeof icons !== 'undefined') {
        icons.universalgpt = logos.universalgpt
    }
    ...
    window.modals = {
        ...
        about() {
            ...
            const aboutModal = modals.alert(
                ...
                + `<span style="${labelStyles}">� This version by:</span> `
                    + `<a href="https://github.com/Processori7" target="_blank" rel="nopener">Processor</a>\n`
            ...
            // Add logo
            const aboutHeaderLogo = logos.universalgpt.create() ; aboutHeaderLogo.width = 420
        ...
        settings: {
            ...
            createAppend() {
                ...
                // Init logo
                const settingsIcon = icons.universalgpt.create()
        ...
        || (config.prefixEnabled || config.suffixEnabled) // or Manual-Gen on
            && [config.prefixEnabled && location.href.includes('text=%2F'), // prefix required/present
                config.suffixEnabled // suffix required/present
                    && /text=.*?(?:%3F|？|%EF%BC%9F)(?:&|$)/.test(location.href)
    ...
    function hideAds() {
        const adSelectors = [
            '.ads', '.serp-adv', '.adv-item', '[data-adv]', '.commercial',
            '.banner', '.promo', '.sponsored', '.ad', '.advertisement',
            '.related-queries', '.also-ask', '.people-also-ask' // and etc.
        ];
